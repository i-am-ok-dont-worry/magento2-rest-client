...
module.exports = function (restClient) {
    ...
    module.updateBulk = function (customerToken, cartId, cartItems) {
      if (customerToken && isNumeric(cartId)) {
    ...
    module.deleteBulk = function (customerToken, cartId, itemIds) {
        if (itemIds) {
            if (customerToken && isNumeric(cartId)) {
    ...
    module.assign = function (cartId, userId, storeId = 0, customerToken) {
    ...
    module.cartInfo = function (customerToken, cartId) {
        if (customerToken) {
            return restClient.get('/carts/mine', customerToken);
        } else {
            return restClient.get('/guest-carts/' + cartId);
        }
    };

    module.selectPaymentMethod = function (customerToken, cartId, paymentMethod) {
        if (customerToken) {
            return restClient.put('/carts/mine/selected-payment-method', paymentMethod, customerToken);
        } else {
            return restClient.put('/guest-carts/' + cartId + '/selected-payment-method', paymentMethod, customerToken);
        }
    };
